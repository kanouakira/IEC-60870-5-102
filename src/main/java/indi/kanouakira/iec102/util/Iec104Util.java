...
import indi.kanouakira.iec102.core.iec104.Iec104MessageDetail;
...
public class Iec104Util {
	 ...
	 * @return
	 */
	public static byte getChangedQualifiers(Iec104MessageDetail ruleDetail104) {
		// 将长度转换成 byte
		byte changedQualifiers = (byte) ruleDetail104.getMessageLength();
		// 判断 SQ 置 isContinuous false SQ = 0;否则 SQ =1 , 同时将SQ置 设置在可变限定词的 D7 位置
	...
	/**
	 * short 转换成两个字节后是163 00 也就是 value[1] 中才有值
	 ...
	 * @return 是否试验
	 */
	public static boolean isTests(byte[] values) {
	...
	public static short getTransferReasonShort(boolean isTests, boolean isYes, short transferReason) {
		int t = isTests ? 1 : 0;
		int y = isYes ? 0 : 1;
