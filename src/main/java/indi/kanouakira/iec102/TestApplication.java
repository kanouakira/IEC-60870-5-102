package indi.kanouakira.iec102;

import indi.kanouakira.iec102.core.Iec102DataConfig;
import indi.kanouakira.iec102.core.Iec102UploadFile;
import indi.kanouakira.iec102.core.enums.TypeIdentificationEnum;
import indi.kanouakira.iec102.server.primary.PrimaryStationFactory;
import indi.kanouakira.iec102.server.secondary.SecondaryStationFactory;
import indi.kanouakira.iec102.util.ByteUtil;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.Calendar;
import java.util.Date;
import java.util.concurrent.Executors;
import java.util.concurrent.TimeUnit;

import static indi.kanouakira.iec102.core.Iec102UploadFile.createUploadFile;
import static indi.kanouakira.iec102.standard.StandardProtocolEnum.IEC102;

/**
 * 测试启动类。
 *
 * @author KanouAkira
 * @date 2022/5/6 16:56
 */
public class TestApplication {
    final static Logger logger = LoggerFactory.getLogger(TestApplication.class);

    public static void main(String[] args) {
        logger.debug("构造一个文件，传入作为测试，启动5分钟后过期");
        // 构造一个文件，传入作为测试，启动5分钟后过期。
        byte[] fileContext = ByteUtil.hexStringToBytes("");
        Calendar instance = Calendar.getInstance();
        instance.setTime(new Date());
        instance.add(Calendar.MINUTE, 5);

        Executors.newScheduledThreadPool(1).scheduleWithFixedDelay(() -> {
            Iec102UploadFile iec102UploadFile = createUploadFile(TypeIdentificationEnum.ZTXX_QXHJ.getValue(), "GF_GZ.ZhongBDC_XB-FZ_20211115_101500.dat", fileContext, instance.getTime());
            Iec102DataConfig.addFile(iec102UploadFile);
        }, 5, 60, TimeUnit.SECONDS);
        SecondaryStationFactory.create(3000, IEC102).run();
        PrimaryStationFactory.create("127.0.0.1", 3000, IEC102).run();
    }
}
